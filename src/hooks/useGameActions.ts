...
export const useGameActions = () => {
  ...
  const createMovingBall = useCallback((selectedBall: Ball, boardRect: DOMRect) => {
    ...
    ballInner.style.boxShadow =
      'inset -3px -3px 5px rgba(0, 0, 0, 0.3), 0 2px 4px rgba(0, 0, 0, 0.2)';

    return { movingBall, actualBallSize };
  }, []);

  const positionBallAtCell = useCallback(
    (
      movingBall: HTMLElement,
      row: number,
      col: number,
      actualBallSize: number,
      boardRect: DOMRect
    ) => {
      const bodyRect = document.body.getBoundingClientRect();

      const targetCell = boardRef.current?.querySelector(
  ...
  const animateStepByStep = useCallback(
    (
      movingBall: HTMLElement,
      path: Position[],
      actualBallSize: number,
  ...
  const animateShowPathThenMove = useCallback(
    (
      movingBall: HTMLElement,
      path: Position[],
      actualBallSize: number,
  ...
  const animateInstantMove = useCallback(
    (
      movingBall: HTMLElement,
      path: Position[],
      actualBallSize: number,
  ...
  const checkForLines = useCallback(
    (currentGrid = grid) => {
  ...
  const handleCellClick = useCallback(
    (row: number, col: number) => {
      ...
      if (selectedCell) {
        ...
        const path = findPath(grid, { row: selectedRow, col: selectedCol }, { row, col });
